...
def final_model(
    ...
    target_dir = os.path.join('.', "Data", "Final_Results")
    os.makedirs(target_dir, exist_ok=True)
    filename = os.path.join(target_dir, f'Final_model_Ridge_Poly_{polynomial_degree}_lambda_{alpha:.4f}.pkl')
...
def single_feature_extraction(
    ...
    target_dir = os.path.join('.', "Data", "Final_Results")
    df.to_csv(os.path.join(target_dir, output_csv), index=False, float_format="%.4f")
...
def multi_model_parameter_extraction(path_to_folder: str, output_csv: str = 'ridge_coefficients.csv'):
    ...
    for fname in sorted(os.listdir(path_to_folder)):
        ...
        target_dir = os.path.join('.', "Data", "Final_Results")
        os.makedirs(target_dir, exist_ok=True)
        df_full.to_csv(os.path.join(target_dir, output_csv), index=False, mode='w' if first_write else 'a', header=first_write)
        first_write = False